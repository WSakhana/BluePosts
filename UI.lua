...
function UI:CreateReader()
    ...
    self.classButton = CreateButton(toolbar, "Classes", "Interface\\Icons\\INV_Misc_Book_11", 136)
...
function UI:RenderPost(post)
    ...
    self.classButton:SetShown(classCount > 0)
...
function UI:SetClassMenuThumbActive(active)
    ...
    local color = active and THEME.gold or THEME.blue
...
function UI:SetClassMenuScrollValue(offset)
    ...
    menu.scroll:SetVerticalScroll(Clamp(offset or 0, 0, maxScroll))
...
function UI:ScrollClassMenu(delta)
    ...
    self:SetClassMenuScrollValue(current - ((delta or 0) * CLASS_MENU_ROW_HEIGHT * 3))
end
...
function UI:SetClassMenuScrollFromThumbTop(thumbTop)
    ...
    local thumbOffset = Clamp(trackTop - thumbTop, 0, travel)
...
function UI:ToggleClassMenu()
    ...
    if not self.classMenu then
        ...
        local child = CreateFrame("Frame", nil, scroll)
        child:SetSize(CLASS_MENU_WIDTH - 14, 1)
        ...
        track.fill:SetColorTexture(THEME.void[1], THEME.void[2], THEME.void[3], 0.65)
        track:Hide()
        ...
        thumb.fill:SetColorTexture(THEME.blue[1], THEME.blue[2], THEME.blue[3], 0.92)
        thumb:Hide()
    ...
    local menu = self.classMenu
    local hasOverflow = #self.classAnchors > CLASS_MENU_MAX_VISIBLE
    local visibleCount = math.min(#self.classAnchors, CLASS_MENU_MAX_VISIBLE)
    local buttonWidth = hasOverflow and (CLASS_MENU_WIDTH - 28) or (CLASS_MENU_WIDTH - 14)

    menu:ClearAllPoints()
    menu:SetPoint("TOPRIGHT", self.classButton, "BOTTOMRIGHT", 0, -6)
    menu:SetSize(CLASS_MENU_WIDTH, (visibleCount * CLASS_MENU_ROW_HEIGHT) + 12)
    ...
    for index, anchor in ipairs(self.classAnchors) do
        ...
        button:SetPoint("TOPLEFT", menu.child, "TOPLEFT", 0, -((index - 1) * CLASS_MENU_ROW_HEIGHT))
